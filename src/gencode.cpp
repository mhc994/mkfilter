...
global const char *progname = NULL;
...
static void usage(), readcmdline(const char* const []);
...
static void pr_xcoeffs(), pr_ycoeffs(), pr_xpart(const char*), prxfac(double), pr_ypart(const char*), prnl();
...
static void pr_xpart(const char *vs)
{
	nxfacs = 0;
...
static void pr_ypart(const char *vs)
{
	for (int i=0; i < npoles; i++)
	{
		if (i%2 == 0) prnl();
